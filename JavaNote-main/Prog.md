...
## 🤔 问题：为什么当线程数 > CPU 核心数，反而效率下降？

~~~markdown
###  一、理想情况（线程数 ≤ CPU 核心数）

- 每个线程都能“独占”一个核心执行，几乎没有等待。
- **无阻塞、无抢占、无切换** → 效率最大化。

------

### ⚠️ 二、现实问题（线程数 > CPU 核心数）

#### 👉 操作系统会触发 “线程上下文切换”（Context Switch）

- CPU 在多个线程之间来回切换，模拟“并发”。
- 每次切换都需要保存/恢复线程状态（如寄存器、程序计数器等），有额外开销。

⛔ **开销包括**：

1. 保存当前线程的上下文
2. 加载下一个线程的上下文
3. 刷新 CPU 缓存（造成 cache miss）
4. CPU pipeline 刷新（指令流水线中断）
...
> 当线程数很多时，CPU 花大量时间“切换”线程，而不是“执行”线程 —— 这就是 **线程过多反而效率低的根本原因**。
...
- 同一个线程在不同时间段运行在不同核心，cache 无法复用。
- 导致频繁访问主内存，效率降低。

#### 3. **内存占用上升**

- 每个线程都需要一定栈空间（Java 默认 1M）
- 线程越多，内存压力越大，甚至 OOM
...
| 任务类型   | 推荐系数     | 示例                   |
| ---------- | ------------ | ---------------------- |
| CPU 密集型 | 核心数 + 1   | 图像处理、大量数学计算 |
| IO 密集型  | 核心数 × 2~3 | 网络、数据库访问       |

------

### ✅ Java 中的做法

使用线程池（如 `ThreadPoolExecutor`）并合理配置参数：
...
> **线程数多 ≠ 并发强，超过 CPU 核心数反而会让系统“忙着切换”，而不是“干活”。**

所以，**线程开多少**不是“越多越好”，而是“刚刚好”。
~~~
